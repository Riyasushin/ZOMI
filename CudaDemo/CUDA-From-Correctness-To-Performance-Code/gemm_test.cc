...
typedef void (*gemm_impl_t)(int* __restrict__ C, const int* __restrict__ A,
                            const int* __restrict__ B, const int n, const int m,
                            const int k);
...
std::vector<GemmImpl> gemm_impls = {
    ...
    {"gpu_mult_block_no_restrict_reg", gemm_gpu_mult_block_no_restrict_reg,
     true},
...
double benchmark_gemm_impl(GemmImpl gemm_impl, const int n, const int m,
                           const int k) {
  // Prepare test data
  ...
  // run_once: Run the gemm_impl once, and return its time usage (in
  // microseconds (us))
